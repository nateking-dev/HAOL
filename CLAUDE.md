...
`POST /tasks` is **asynchronous**: it validates the input, inserts a `task_log` row in `QUEUED` status, hands the job to the in-process worker (`src/services/task-worker.ts`), and returns `202 Accepted` with `{ task_id, status: "QUEUED", links.self }` plus a `Location` header. Clients poll `GET /tasks/:id` until `done: true`. The worker drains the queue with bounded concurrency (`WORKER_CONCURRENCY`); on crash, the reaper (`src/services/task-reaper.ts`) re-enqueues stranded `QUEUED` rows and marks rows stuck in `RECEIVED/CLASSIFIED/DISPATCHED` past `WORKER_RECOVERY_AGE_MS` as `FAILED` with `worker_error="worker_crashed"`. Each reaper sweep also enforces PII retention: raw `task_log.prompt` and `routing_log.input_text` older than `PROMPT_RETENTION_DAYS` (default 30, `0` disables) are nulled, leaving the `prompt_hash`/`input_text_sha256` fingerprints intact. The `/observability/cascade?include_text=true` raw-text view additionally requires `ALLOW_PROMPT_DISCLOSURE=1`, otherwise it fails closed to fingerprints only. **Limitation:** the purge nulls live rows only — Dolt commit history still contains the original text and is readable via `dolt log`/`dolt diff`/`AS OF` queries, so this bounds disclosure through the SQL/API surface, not true erasure.
